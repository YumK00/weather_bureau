...
int main(int argc,char *argv[])
{
    ...
    if (argc!=5)
    {
        ...
        printf("Example:/project/tools/bin/procctl 120 /A/idc/bin/obtcodetodb /A/idc/ini/stcode.ini "\
                  "\"idc/idcpwd@snorcl11g_5\" \"Simplified Chinese_China.AL32UTF8\" /A/log/idc/obtcodetodb.log\n\n");
...
//     char height[11];
// }zhandian;
// list<st_zhandian>zdrq;
// bool loadcode(const string &inicode);//把ini数据存到容器中
...
//         return -1;
//     }
//     if(loadcode(argv[1])==false){
//         logfile.write("loadcode失败.\n");
//         return -1;
//     }
...
//     sqlstatement statinsert(&conn);
//     statinsert.prepare("insert into T_ZHOBTCODE(obtid,cityname,provname,lat,lon,height,keyid)\
//                     values(:1,:2,:3,:4*100,:5*100,:6*10,SEQ_ZHOBTCODE.nextval)");
//     statinsert.bindin(1,zhandian.obtid,5);
//     statinsert.bindin(2,zhandian.cityname,30);
//     statinsert.bindin(3,zhandian.province,30);
//     statinsert.bindin(4,zhandian.lat,10);
//     statinsert.bindin(5,zhandian.lon,10);
//     statinsert.bindin(6,zhandian.height,10);

//     sqlstatement statalter(&conn);
//     statalter.prepare("update T_ZHOBTCODE set cityname=:1,provname=:2,lat=:3*100,lon=:4*100,height=:5*10\
//                      where obtid=:6");
//     statalter.bindin(1,zhandian.cityname,30);
//     statalter.bindin(2,zhandian.province,30);
//     statalter.bindin(3,zhandian.lat,10);
//     statalter.bindin(4,zhandian.lon,10);
//     statalter.bindin(5,zhandian.height,10);
//     statalter.bindin(6,zhandian.obtid,5);

//     int insertnum=0,alternum=0;
//     for(auto &aa:zdrq){
//         zhandian=aa;
//         if(statinsert.execute()!=0){
//             if(statinsert.rc()==1)//违反pk唯一,就是表里面有这个不插入,查看需不需要更新
//             {
//                 if(statalter.execute()!=0){
//                     logfile.write("statalter.execute() failed.\n%s\n%s\n",statalter.sql(),statalter.message());
//                     EXIT(-1);
//                 }
//                 else{
//                     alternum++;
...
//             else{
//                 // 其它错误，程序退出。
//                 logfile.write("statinsert.execute() failed.\n%s\n%s\n",statinsert.sql(),statinsert.message());
//                 EXIT(-1);
//             }
//         }
//         else{
...
//         }
//         logfile.write("provname=%s,obtid=%s,cityname=%s,lat=%s,lon=%s,height=%s\n",\
//                   zhandian.province,zhandian.obtid,zhandian.cityname,zhandian.lat,zhandian.lon,zhandian.height);
//     }
//     logfile.write("总记录数=%d,插入记录=%d,更新记录=%d\n",zdrq.size(),insertnum,alternum);
//     conn.commit();
//     return 0;
// }
...
// void EXIT(int sign)
// {
//     logfile.write("signal = %d",sign);
//     exit(0);
// }

// bool loadcode(const string &inicode)
// {
//     cifile ifile;
//     if(ifile.open(inicode)==false){
...
//     while (true)
//     {
//         if(ifile.readline(buff)== false)    break;
//         cmdstr.splittocmd(buff,",");
//         memset(&zhandian,0,sizeof(struct st_zhandian));
//         cmdstr.getvalue(0,zhandian.province,30);
//         cmdstr.getvalue(1,zhandian.obtid,5);
//         cmdstr.getvalue(2,zhandian.cityname,30);
//         cmdstr.getvalue(3,zhandian.lat,10);
//         cmdstr.getvalue(4,zhandian.lon,10);
//         cmdstr.getvalue(5,zhandian.height,10);
//         zdrq.push_back(zhandian);
        
//     }
//     // for(auto &aa:zdrq){
//     //     logfile.write("provname=%s,obtid=%s,cityname=%s,lat=%s,lon=%s,height=%s\n",\
//     //               aa.province,aa.obtid,aa.cityname,aa.lat,aa.lon,aa.height);
//     // }
