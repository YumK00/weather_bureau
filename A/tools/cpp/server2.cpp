...
bool senddata(const int sockfd,const char *strget)
{
    ...
    // 查询条件：设计接口的时候决定。                                           可读性不好，可扩展性也不好。
    // http://192.168.150.128:8080/api?wucz&wuczpwd&getZHOBTMIND1&51076&20211024094318&20211024114020
    // http://192.168.150.128:8080/api?username=idc&passwd=idcpwd&intername=getZHOBTMIND1&obtid=51076&begintime=20230707000000&endtime=20230708000000
    ...
    sqlstatement stmt(&conn);
    int a = stmt.prepare(
            "select '<obtid>'||obtid||'</obtid>'||'<datetime>'||to_char(datetime,'yyyy-mm-dd hh24:mi:ss')||'</datetime>'||'<t>'||t||'</t>'||'<p>'||p||'</p>'||'<u>'||u||'</u>'||'<keyid>'||keyid||'</keyid>'||'<endl/>' "
            "from T_ZHOBTMIND");
    //int a = stmt.prepare("select * from T_ZHOBTMIND");
    if(a!=0){
        cout<<"failed.\n";
        return false;
    }
...
int main(int argc,char* argv[]){
    ...
    addr.sin_port = htons(atoi(argv[1]));
     if(bind(lisenfd,(struct sockaddr*)&addr,sizeof(addr))!=0){
        cout<<"bind failed.\n";
        close(lisenfd);
        return -1;
     }
     ...
     int serverfd = accept(lisenfd,0,0);
     if(serverfd<=0){
        cout<<"accept failed.\n";
        close(lisenfd);
        return -1;
     ...
     if((recv(serverfd,recvbuff,sizeof(recvbuff),0))<=0){
        cout<<"recv failed.\n"<<endl;
        return -1;
     }
     cout<<recvbuff<<endl;
    ...
    send(serverfd,sendbuff,strlen(sendbuff),0);
    senddata(serverfd,recvbuff);
    close(lisenfd);
    close(serverfd);
    return 0;
